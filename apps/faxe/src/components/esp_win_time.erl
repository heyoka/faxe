...
tick(State = #esp_window{stats = #esp_win_stats{events = {[],[],[]} } }) ->
   State;
tick(State = #esp_window{agg_mod = Module, agg = Agg, every = Every, agg_fields = Field,
   ...
   {{Tss, Vs, _Es}=Keep, _Evict} = evict(Events, At, Interval),
...
%%   lager:notice("on tick timespan is ~p | At-Mark is: ~p | AT : ~p",[NewAt-hd(Tss), NewAt-Mark, faxe_time:to_date(NewAt)]),
   case (NewAt - Mark) >= Every of
      true -> %{ok, Res, AggS} = Module:emit(Stats, NewAgg),
         ...
         lager:warning("~n when ~p ~p emitting: ~p",[NewAt-Mark, ?MODULE, {Res, length(Res#data_batch.points)}]),
         dataflow:emit(Res),
