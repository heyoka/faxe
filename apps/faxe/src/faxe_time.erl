...
-export_type([timestamp/0, unit/0, interval/0, duration/0]).
...
%% @end
%%
-spec add(timestamp(), binary()) -> timestamp().
...
%% @doc
%% get a timestamp, where the unit is set to the nearest (past or present) interval occurance
%% @end
...
%%   lager:info("Distance:: ~p",[Distance]),
   ...
   D = set_to_unit(Unit, datetime_parts(Ts), NewUnitValue),
%%   lager:info("Date : ~p",[D]),
...
get(millisecond, Ts) ->
   {_Date,{_H,_Min,_Second,Milli}} = datetime_parts(Ts),
   Milli;
get(second, Ts) ->
   {_Date,{_H,_Min,Second,_M}} = datetime_parts(Ts),
   Second;
get(minute, Ts) ->
   {_Date,{_H,Minute,_S,_M}} = datetime_parts(Ts),
   Minute;
get(hour, Ts) ->
   {_Date,{H,_Min,_S,_M}} = datetime_parts(Ts),
   H;
get(day_of_week, Ts) ->
   {Date,_T} = datetime_parts(Ts),
   calendar:day_of_the_week(Date);
get(day_of_week_string, Ts) ->
   httpd_util:day(get(day_of_week, Ts));
get(day, Ts) ->
   {{_Y, _M, Day},_T} = datetime_parts(Ts),
   Day;
get(month, Ts) ->
   {{_Y, Month, _D},_T} = datetime_parts(Ts),
   Month;
get(month_string, Ts) ->
   httpd_util:month(get(month, Ts));
get(year, Ts) ->
   {{Year, _M, _D},_T} = datetime_parts(Ts),
   Year.
