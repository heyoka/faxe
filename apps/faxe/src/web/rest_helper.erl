...
%% API
-export([task_to_json/1, template_to_json/1]).
...
task_to_json(T = #task{definition = Def0, id = Id, name = Name, date = _Dt}) ->
...
%%   Fields = record_info(fields, T),
%%   Def = maps:from_list(Def0),
   Map = #{id => Id, name => Name, dfs => Dfs, date => faxe_time:to_hdate(Dt)},
